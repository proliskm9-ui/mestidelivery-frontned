...
import { adminApi, adminAuth, type Product } from '../../services/adminService';
...
export function AdminStoreProducts() {
    ...
    const loadProducts = async (storeId: string) => {
        ...
        try {
            ...
            const allProducts = await adminApi.get<Product[]>(`/products/?restaurant_id=${storeId}&limit=1000&_t=${Date.now()}`, true);
    ...
    const handleSaveProduct = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!editProduct || !selectedStore) return;
    ...
    const handleSaveStoreInfo = async () => {
        ...
        try {
            ...
            // Update local state
            setStores(prev => prev.map(r => r.id === editStore.id ? { ...r, ...editStore } as Store : r));
            setSelectedStore(prev => prev?.id === editStore.id ? { ...prev, ...editStore } as Store : prev);
    ...
    const filteredStores = stores.filter(r => r.name.toLowerCase().includes(searchTerm.toLowerCase()));
        ...
        <div className="admin-page">
            ...
            <div className="page-header" style={{ flexWrap: 'wrap', gap: '12px' }}>
                <div style={{ display: 'flex', alignItems: 'stretch', gap: '12px', flex: 1, minWidth: '250px' }}>
                    ...
                    <div style={{ position: 'relative', flex: 1, maxWidth: '400px', display: 'flex', flexDirection: 'column' }}>
                        ...
                        >
                            <div style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                                {selectedStore?.img ? (
                                    <img src={selectedStore.img} alt="" style={{ width: 28, height: 28, borderRadius: 8, objectFit: 'cover' }} />
                                ) : (
                            ...
                            <>
                                ...
                                <div className="store-dropdown-panel">
                                    ...
                                    {filteredStores.map(r => {
                                        const isActive = selectedStore?.id === r.id;
                                        return (
                                            <div
                                                key={r.id}
                                                onClick={() => selectStore(r)}
                                            ...
                                            >
                                                ...
                                                <div>
                                                    ...
                                                    <div style={{ fontSize: '12px', color: '#888' }}>{r.delivery}</div>
                                                </div>
                                            </div>
                                        );
                                    })}
                    ...
                    {selectedStore && (
                        <button
                            className="restaurant-selector-btn"
                            onClick={() => { setEditStore({ ...selectedStore }); setShowStoreInfo(!showStoreInfo); }}
                ...
                <div style={{ display: 'flex', gap: '10px' }}>
                    <button className="admin-btn" onClick={() => selectedStore && loadProducts(selectedStore.id)} title="Обновить">
                        <RefreshIcon size={18} />
                    </button>
                    <button className="admin-btn admin-btn-primary" onClick={openNewProduct} disabled={!selectedStore}>
                        Добавить блюдо
                    </button>
                </div>
            </div>

            {/* ─── Store Info Panel (collapsible) ─── */}
            {showStoreInfo && editStore && (
                <div className="admin-card" style={{ marginBottom: '20px', padding: '24px' }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
                        <h3 style={{ margin: 0, fontSize: '16px', fontWeight: 700 }}>📋 Информация о магазине</h3>
                        <button className="admin-btn" style={{ fontSize: '12px', padding: '6px 12px' }} onClick={() => setShowStoreInfo(false)}>Свернуть</button>
                    </div>
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(220px, 1fr))', gap: '16px' }}>
                        <div className="form-group">
                            <label className="form-label">Название</label>
                            <input className="admin-input" value={editStore.name || ''} onChange={e => setEditStore({ ...editStore, name: e.target.value })} />
                        </div>
                        
                        <div className="form-group">
                            <label className="form-label">Время доставки</label>
                            <input className="admin-input" value={editStore.delivery || ''} onChange={e => setEditStore({ ...editStore, delivery: e.target.value })} />
                        </div>
                        
                        <div className="form-group" style={{ gridColumn: 'span 2' }}>
                            <label className="form-label">Фото (URL)</label>
                            <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
                                {editStore.img && <img src={editStore.img} alt="" style={{ width: 48, height: 48, borderRadius: 12, objectFit: 'cover' }} />}
                                <input
                                    className="admin-input"
                                    style={{ flex: 1 }}
                                    value={editStore.img || ''}
                                    ...
                                    onChange={async (e) => {
                                        if (e.target.files?.[0]) {
                                            try {
                                                ...
                                                if (res.success) setEditStore(prev => prev ? { ...prev, img: res.url } : null);
                    ...
                    <div style={{ marginTop: '16px', display: 'flex', gap: '10px', justifyContent: 'flex-end' }}>
                        <button className="admin-btn" onClick={() => setShowStoreInfo(false)}>Отмена</button>
                        <button className="admin-btn admin-btn-primary" onClick={handleSaveStoreInfo} disabled={savingStore}>
                            {savingStore ? 'Сохранение...' : 'Сохранить изменения'}
                        </button>
                    </div>
                </div>
            )}
            ...
            {selectedStore ? (
                ...
                <div className="admin-card" style={{ padding: '60px 20px', textAlign: 'center' }}>
                    ...
                    <h3 style={{ margin: '0 0 8px 0', fontWeight: 700 }}>Выберите магазин</h3>
                    <p style={{ color: '#888', margin: 0 }}>Используйте выпадающий список выше, чтобы выбрать магазин и управлять его меню</p>
                </div>
            )}
