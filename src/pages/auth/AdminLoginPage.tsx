...
/** Firebase email/password admin login — no registration, no Google */
const AdminLoginPage: React.FC = () => {
  const { t } = useLanguage();
  const navigate = useNavigate();
  const { signInAdmin, role, loading: authLoading, errorKey, clearError, user } = useAuth();
  ...
  return (
    <div className="staff-login-page">
      <div className="staff-login-card">
        <h1 className="staff-login-title">{t('auth.admin_login_title')}</h1>
        <p className="staff-login-subtitle">{t('auth.admin_login_desc')}</p>
