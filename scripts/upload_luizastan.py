# -*- coding: utf-8 -*-
"""Recreate Luizastan and upload products via POST /products (is_available=true)."""
...
def main() -> None:
    ...
    for r in restaurants:
        if str(r.get("name", "")).lower() == "luizastan":
            rid = r["id"]
            print(f"deleting old restaurant {rid}")
            req("DELETE", f"/restaurants/{rid}", token=token)
    ...
    rest_id = rest["id"]
    print(f"restaurant created status={status} id={rest_id}")
    ...
    print(json.dumps({"restaurant_id": rest_id, "ok": ok, "fail": fail, "visible": len(check)}, ensure_ascii=False))
    if check:
        print("sample:", check[0].get("name"))
